...
public class PhotonVisionSim extends SubsystemBase implements VisionIO {
  ...
  public List<Transform3d> getTransformListSim() {
    Logger.recordOutput(
        "OrthogonalToTag/currentPose Before Processing", RobotState.getInstance().getPose());

    visionSim
        .getVisionTargets()
        .forEach(
            target -> {
              Logger.recordOutput("OrthogonalToTag/targetPose Before Processing", target.getPose());
              System.out.println(target.fiducialID);
            });

    return visionSim.getVisionTargets().stream()
        .map(target -> target.getPose().minus(new Pose3d(RobotState.getInstance().getPose())))
        .collect(Collectors.toList());
  }
